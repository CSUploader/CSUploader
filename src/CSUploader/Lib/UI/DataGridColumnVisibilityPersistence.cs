...
/// <para>
/// Persisted format identical to the WPF head's
/// <c>src/Lib/UI/DataGridColumnVisibilityPersistence.cs</c> — both heads read the same
/// Setting rows; never change one side alone. The DB-facing halves
/// (<see cref="LoadOverridesAsync"/>, <see cref="SaveOverridesAsync"/>, the separator
/// constants, and <see cref="ColumnState"/>) are copied verbatim from that file; only the
/// grid-facing members are re-implemented on Avalonia's <see cref="DataGrid"/> API. The
/// format-vector tests are duplicated across both suites so a drift on either side breaks
/// that side's tests.
/// </para>
/// </summary>
public static class DataGridColumnVisibilityPersistence
{
    ...
    /// <summary>
    /// Persisted state for a single column. <see cref="DisplayIndex"/> mirrors
    ...
    /// <see cref="ApplyAsync"/> so unit tests don't need a realized DataGrid.
    /// </summary>
    ...
    public static async Task ApplyAsync(DataGrid grid, SettingRepository repo, string settingKey, CancellationToken cancellationToken = default)
    {
        ...
        foreach (DataGridColumn column in grid.Columns)
        {
            ...
            {
                column.IsVisible = state.Visible;
            }
        }
    ...
    /// <see cref="DataGrid.Columns"/> rather than <c>DisplayIndex</c> — at capture time
    /// Avalonia may not have finalized DisplayIndex (and a live row-group spacer column
    /// offsets it), so the collection index is the reliable source of "the order the
    /// developer wrote in XAML".
    /// </summary>
