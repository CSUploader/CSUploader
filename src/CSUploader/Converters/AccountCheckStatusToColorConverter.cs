...
/// grid. Resolves the brush against the theme variant active at conversion time — not a
/// hard-coded RGB legible on only one theme. A live theme flip does NOT re-run the converter
/// (a value converter re-runs on its bound source changing, not on the DynamicResource it
/// reads changing): rows repaint as their status changes or the row recycles, not on the flip.
/// </summary>
/// <remarks>
