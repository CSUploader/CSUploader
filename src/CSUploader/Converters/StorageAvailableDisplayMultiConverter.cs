...
///   retrieve): "-".</item>
/// </list>
/// Avalonia <see cref="IMultiValueConverter"/>: values arrive as an <c>IList&lt;object?&gt;</c>; an
/// unset/null slot fails the <c>is long</c> / <c>as string</c> match and is treated as "not provided".
/// </summary>
public class StorageAvailableDisplayMultiConverter : IMultiValueConverter
{
    ...
    {
        long? used = values.Count > 0 && values[0] is long u ? u : null;
        long? quota = values.Count > 1 && values[1] is long q ? q : null;
        string? hosterName = values.Count > 2 ? values[2] as string : null;
