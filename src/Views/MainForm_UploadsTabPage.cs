...
public partial class MainForm : Form
{
    ...
    private readonly Tuple<PackageJob, JobStatus, string>[] statusImages =
    [
    ...
    public void MainForm_UploadsTabPage_Load(object? sender, EventArgs e)
    {
        ...
        // Timers
        tmrUploadsTabPageRefresh.Interval = _settings.UploadsTabPageRefreshTimer * 1000;
        ...
        olvUploadsName.ImageGetter = (object rowObject) =>
        {
            if (rowObject is Package package)
            {
                ...
                {
                    return ilPackage.Images.Keys
                        .Cast<string>()
                        .Where(k => k.StartsWith("package_compressing.", StringComparison.Ordinal))
                        .Select(k => ilPackage.Images[k])
                        .FirstOrDefault(img => img != null);
                }

                return ilPackage.Images.Keys
                    .Cast<string>()
                    .Where(k => k.StartsWith($"package_{(tlvUploads.IsExpanded(rowObject) ? "open" : "closed")}.", StringComparison.Ordinal))
                    .Select(k => ilPackage.Images[k])
                    .FirstOrDefault(img => img != null);
            }
            else if (rowObject is PackageFile packageFile)
            {
                return ilFileTypes.Images.Keys
                    .Cast<string>()
                    .Where(s => s.StartsWith($"filetype_{packageFile.FileType}.", StringComparison.OrdinalIgnoreCase))
                    .Select(k => ilFileTypes.Images[k])
                    .FirstOrDefault(img => img != null);
            }
        ...
        olvUploadsHoster.AspectGetter = (object rowObject) =>
        {
            if (rowObject is Package package)
            {
                ...
                {
                    var fileHosterImages = ilFileHosters.Images.Keys
                        .Cast<string>()
                        .Where(k => k.StartsWith($"filehoster_{fileHoster.Name}.", StringComparison.OrdinalIgnoreCase))
                        .Select(k => ilFileHosters.Images[k])
                        .Where(img => img != null)
                        .ToList();
                    
                    images.AddRange(fileHosterImages!);
                }
            ...
            {
                return ilFileHosters.Images.Keys
                    .Cast<string>()
                    .Where(k => k.StartsWith($"filehoster_{packageFile.FileHosters.First().Name}.", StringComparison.OrdinalIgnoreCase))
                    .Select(k => ilFileHosters.Images[k])
                    .FirstOrDefault(img => img != null);
            }
        ...
        olvUploadsStatus.ImageGetter = (object rowObject) =>
        {
            if (rowObject is Package package)
            {
                ...
                foreach (PackageFile packageFile in package)
                {
                    ...
                    var statusImage = ilStatus.Images.Keys
                        .Cast<string>()
                        .Where(s => s.StartsWith($"{key}.", StringComparison.OrdinalIgnoreCase))
                        .Select(k => ilStatus.Images[k])
                        .FirstOrDefault(img => img != null);
                        
                    if (statusImage != null && !images.Contains(statusImage))
                    {
                        images.Add(statusImage);
                    }
                }
