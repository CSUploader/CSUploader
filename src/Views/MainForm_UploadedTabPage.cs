...
{
    private List<UploadPackageDto> UploadPackages { get; set; } = [];
    ...
    public void MainForm_UploadedTabPage_Load(object sender, EventArgs e)
    {
        ...
        olvUploadedSize.AspectGetter = (object rowObject) =>
        {
            if (rowObject is UploadPackageDto uploadPackage)
            {
                return ByteUnit.FromBytes(uploadPackage.Files.Sum(f => f.FileSize), ByteBase.Binary).ToFriendlyString();
            }
            else if (rowObject is UploadPackageFileDto uploadPackageFile)
            {
                return uploadPackageFile.FileName;
        ...
        olvUploadedHoster.AspectGetter = (object rowObject) =>
        {
            if (rowObject is UploadPackageDto uploadPackage)
            {
                ...
                foreach (UploadPackageFileDto uploadPackageFile in uploadPackage.Files)
                {
                    var fileHosterImages = ilFileHosters.Images.Keys
                        .Cast<string>()
                        .Where(k => k.StartsWith($"filehoster_{uploadPackageFile.FileHosterName}.", StringComparison.OrdinalIgnoreCase))
            ...
            else if (rowObject is UploadPackageFileDto uploadPackageFile)
            {
                return ilFileHosters.Images.Keys
                    .Cast<string>()
                    .Where(k => k.StartsWith($"filehoster_{uploadPackageFile.FileHoster}.", StringComparison.OrdinalIgnoreCase))
                    .Select(k => ilFileHosters.Images[k])
        ...
        olvUploadedDuration.AspectGetter = (object rowObject) =>
        {
            if (rowObject is UploadPackageDto uploadPackage)
            {
                DateTime firstDateTime = uploadPackage.Files.OrderBy(f => f.StartDateTime).Select(f => f.StartDateTime).First();
                DateTime lastDateTime = uploadPackage.Files.OrderBy(f => f.FinishedDateTime).Select(f => f.FinishedDateTime).First();
        ...
        olvUploadedFileCount.AspectGetter = (object rowObject) => (rowObject is UploadPackageDto uploadPackage) ? uploadPackage.Files.Count as int? : null;
    ...
    {
        UploadPackageDto[] packages = await UploadPackageManager.GetAllAsync(cancellationToken);
