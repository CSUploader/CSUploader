...
namespace CSUploader.Views
{
    ...
    {
        private List<FileHosterListViewModel> Models { get; set; } = new();
        ...
        private void TbUploadInputPackageNamingExpression_TextChanged(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < match.Groups.Count; i++)
                {
                    ...
                    result = result.Replace("{" + i + "}", g.Value);
                }
        ...
        {
            string initialDirectory = tbUpload7zOutputDirectory.TextLength > 0 ? tbUpload7zOutputDirectory.Text : Settings.TempArchiveDirectory;
            CommonOpenFileDialog dialog = new()
            {
                InitialDirectory = initialDirectory,
                IsFolderPicker = true
        ...
        private void BtnUploadUpload_Click(object? sender, EventArgs e)
        {
            ...
            foreach (string directory in directories)
            {
                ...
                try
                {
                    Package package = packageManager.AddPackage(options);
                    packageManager.StartPackage(package);
                }
                catch (Exception ex)
                {
        ...
        {
            Models = new List<FileHosterListViewModel>();
            ...
            {
                if (Database == null || fileHoster == null)
                {
                    continue;
                }
                ...
                FileHosterListViewModel model = new()
                {
                    ...
                    Accounts = await Database.FindFileHosterLoginsAsync(fileHoster.Name, cancellationToken)
                };
        ...
        private void FindDirectories(string directoryPath, Regex expression, List<string> directories)
        {
