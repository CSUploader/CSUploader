...
#pragma warning restore SA1300 // Element should begin with upper-case letter
        ...
        private static Dictionary<ByteUnit, Tuple<ByteUnitPrefix, ByteUnitSymbol>> ByteUnits { get; } = new Dictionary<ByteUnit, Tuple<ByteUnitPrefix, ByteUnitSymbol>>();
        ...
        private static string ParseNumber(string number)
        {
            ...
            int firstDotIndex = number.IndexOf('.');
            if (firstCommaIndex >= 0 && firstDotIndex >= 0)
            {
                ...
                // etc.
                string[] values = number.Replace(groupSeperator.ToString(), string.Empty).Split(new[] { fractionSeperator });
            ...
            if (firstCommaIndex >= 0 || firstDotIndex >= 0)
            {
                ...
                if (regex.IsMatch(number))
                {
                    ...
                    // ,5 -or .5
                    // 5, -or- 5.
                    string[] values = number.Split(new[] { seperator });
                    string decimals = string.IsNullOrEmpty(values[0]) ? "0" : values[0];
                ...
                // 121,152 -or- 121.152
                return number.Replace(regexSeperator.ToString(), string.Empty);
            }
