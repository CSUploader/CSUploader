...
namespace CSUploader.Upload
{
    ...
    public class RapidgatorClient : FileHosterClient
    {
        ...
        {
            FileUploadRequestResponse? fileUploadRequestResponse = await HttpUploadFileRequestAsync(userInfoResponse, folderCreateResponse, filePath, cancellationToken);
        ...
        private async Task<FileUploadRequestResponse?> HttpUploadFileRequestAsync(UserInfoResponse userInfoResponse, FolderCreateResponse folderCreateResponse, string filePath, CancellationToken cancellationToken = default)
        {
            ...
            return await HttpSendReceiveAsync<FileUploadRequestResponse>(link, "upload file", cancellationToken);
        }
            ...
            where T : class
        {
            ...
            {
                Logger.Log(this, LogType.Error, $"[{nameof(RapidgatorClient)}] Failed to send HTTP GET: {ex}");
                throw;
            }
            ...
            if (JsonHelpers.TryDeserializeObject(result, out Response<T>? modelResponse))
            {
                ...
                {
                    Logger.Log(this, LogType.Error, $"[{nameof(RapidgatorClient)}] Failed to {action}: {modelResponse?.Status} {modelResponse?.Details}");
                    return null;
                }
            ...
            {
                Logger.Log(this, LogType.Error, $"[{nameof(RapidgatorClient)}] Failed to {action}:{Environment.NewLine}"
                    + $"Response Status: {response?.Status}{Environment.NewLine}"
                    + $"Response details: {response?.Details}{Environment.NewLine}");
            }
            else
            {
                Logger.Log(this, LogType.Error, $"[{nameof(RapidgatorClient)}] Action '{action}' failed to execute: {result}");
            }
        ...
        private void HttpHandler_UploadFinished(object? sender, HttpUploadFinishedEventArgs e)
        {
            ...
            if (JsonHelpers.TryDeserializeObject(e.Result, out Response<FileUploadResponse>? fileUploadResponse) && !string.IsNullOrEmpty(fileUploadResponse.Details))
            {
                ...
                {
                    Logger.Log(this, LogType.Error, $"[{nameof(RapidgatorClient)}] Failed to upload file: {fileUploadResponse.Status} {fileUploadResponse.Details}");
                    eventArgs = new FileHosterUploadFinishedEventArgs(false, fileUploadResponse.Details, startDateTime);
                }
                else if (fileUploadResponse.Model?.Upload != null && fileUploadResponse.Model.Upload.File.Any())
                {
            ...
            {
                Logger.Log(this, LogType.Error, $"[{nameof(RapidgatorClient)}] Failed to upload file: {response.Status} {response.Details}");
