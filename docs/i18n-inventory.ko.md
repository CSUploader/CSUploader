...
Settings_General_CheckForUpdatesAtStartup = 시작할 때 업데이트 확인
...
Settings_Upload_MaxParallelParts   = 파일당 최대 병렬 파트 수
Settings_Upload_MaxParallelPartsTip= 파트를 순서에 상관없이 보낼 수 있는 프로토콜을 가진 호스터에만 적용됩니다. 종량제이거나 불안정한 연결에서는 낮추세요.
...
Settings_Upload_DefaultUploadDirectory             = 기본 업로드 디렉터리
Settings_Upload_DefaultUploadDirectory_Watermark   = 마지막으로 찾아본 위치 열기
Settings_Upload_DefaultUploadDirectoryTip          = “파일 추가…”와 “폴더 추가…”가 열리는 위치입니다. 비워 두면 마지막으로 찾아본 위치가 열립니다.
Settings_Upload_DefaultUploadDirectory_DialogTitle = 기본 업로드 디렉터리 선택
```
...
Wizard_Step2_Col_Captcha           = 다운로드 캡차?
Wizard_Step2_Col_Captcha_Tooltip   = 계정 없이 공유 링크를 다운로드할 때 캡차를 풀어야 하는지 여부입니다. 대기 시간과 브라우저 자동 검사는 포함되지 않습니다.
Wizard_Step2_Captcha_Unknown       = —
Wizard_Step2_Captcha_UnknownTooltip = 이 호스트에서 다운로드할 때 캡차가 필요한지는 확인되지 않았습니다.
...
Upload_SkippedNotSignedIn_Format = 건너뜀 — {0}에 로그인되어 있지 않습니다. 설정 → 계정에서 로그인한 후 업로드를 다시 시작하세요.
Accounts_SessionExpired                    = 세션이 만료되었습니다. 다시 로그인하세요.
Wizard_Step2_SessionExpiredTooltip         = 이 계정의 로그인이 만료되었습니다. 설정 → 계정에서 다시 로그인하세요.
Wizard_Hoster_AccountSessionExpired_Format = {0}: "{1}"의 로그인이 만료되어 업로드되지 않습니다. 설정 → 계정에서 다시 로그인하세요.
Wizard_Step2_CheckAllTip = 목록의 모든 호스터를 선택하거나 해제합니다. 필터를 따르며, 계정이 필요한 호스터는 제외합니다.
...
Wizard_AccountCheckFailed_Format   = {0}에 로그인할 수 없어 계정을 추가하지 않았습니다: {1}    # {0} = hoster name, {1} = verifier message
