...
Uploads_Overview_Elapsed          = Elapsed
Uploads_Overview_FinishAt         = Finish at
...
Uploads_Overview_ElapsedLabel         = Elapsed:
Uploads_Overview_FinishAtLabel        = Finish at:
...
Uploads_Reset_Multi_Format           = Reset {0} selected item(s)? {1} already uploaded successfully — resetting will re-hash and re-upload them.  # {0} = selected count, {1} = completed count
...
Settings_Upload_MaxParallelParts   = Max. parallel parts per file
Settings_Upload_MaxParallelPartsTip= Only applies to hosters whose protocol allows a file's parts to be sent out of order. Lower this on a metered or unreliable connection.
...
Wizard_Step2_Col_Captcha           = Download captcha?
Wizard_Step2_Col_Captcha_Tooltip   = Whether someone downloading a shared link without an account must solve a captcha. Wait timers and automatic browser checks don't count.
Wizard_Step2_Captcha_Unknown       = —
Wizard_Step2_Captcha_UnknownTooltip = This host's download-captcha requirement couldn't be verified.
...
Upload_SkippedNotSignedIn_Format = Skipped — not signed in to {0}. Sign in under Settings → Accounts, then start the upload again.
Accounts_SessionExpired                    = Session expired — sign in again.
Wizard_Step2_SessionExpiredTooltip         = This account's sign-in has expired. Open Settings → Accounts and sign in again.
Wizard_Hoster_AccountSessionExpired_Format = {0}: the sign-in for "{1}" has expired, so nothing will be uploaded there — sign in again under Settings → Accounts.
