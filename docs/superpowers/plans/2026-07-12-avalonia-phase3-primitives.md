...
7. **`DataGridColumn.Header` live-update on indexer invalidation** — the property KIND is resolved (reflection-verified against 11.3.13: `DirectProperty<DataGridColumn, object>`, bindable, OneWay default); Task 7 Step 4 answers only whether the binding re-evaluates on `PropertyChanged("Item[]")`; both outcomes recorded, port rule updated. **RESOLVED at Task 7 (2026-07-11):** a `{Binding [key], Source=Localizer.Instance}` **indexer** binding does NOT live-update on `Localizer`'s `PropertyChanged("Item[]")`/`("")` — verified empirically (isolation probes: a plain-property binding updates on its notification; both a programmatic indexer binding to `Localizer` and an indexer binding to a throwaway INPC double failed to re-read on `"Item[]"`+`""`). This affected BOTH the styled `TextBlock.Text` case (Step 3) and the `DataGridColumn.Header` DirectProperty case (Step 4) — the initial value resolved but never updated. FIX (not the code-behind fallback): `LocExtension` binds to an **observable** that re-emits `Localizer.Instance[key]` on every culture change (`IObservable<object?>.ToBinding()`), sidestepping the indexer-INPC path. With that, the DirectProperty header both **attaches AND live-updates** (headless test `CultureSwitch_ReEvaluatesLocBinding_OnDirectProperty` green; bridge-verified en→ja→en retitle on the live head). **Phases 5/6 port rule:** no code-behind header rebind is needed — `{loc:Loc key}` works uniformly on styled and direct properties. **Leak caveat (review follow-up, 2026-07-11):** the observable's `Localizer.PropertyChanged` handler MUST hold the observer WEAKLY. Avalonia disposes an observable-binding subscription only on an explicit unbind, which virtualized DataGrid rows and TabControl content regeneration never perform — a strong handler pins every bound control graph onto the process-lifetime singleton (proven: N bindings → N handlers surviving teardown+GC). `LocExtension.WeakSubscription` holds a `WeakReference` to the observer and self-prunes a dead one on the next culture switch (regression test `TornDownBinding_DoesNotLeakOntoLocalizerSingleton`: subscriber count 0→1 on bind, back to 0 after teardown+GC+switch; still live-updates). A reimplementation must keep the weak-handler property.
...
10. **Markup-extension duck typing with ctor argument** — `{loc:Loc Main_Tab_Uploads}` compiling via XamlX with the `LocExtension(string)` ctor (Task 7); if positional-arg resolution balks, the `Key` property-setter form `{loc:Loc Key=…}` is the fallback and every ported XAML site must then be adjusted — surface it loudly, don't absorb it silently. **RESOLVED at Task 7 (2026-07-11):** positional-arg resolution works — `{loc:Loc Main_Tab_*}` binds cleanly in both the four MainWindow headers (live head, no binding errors) and the runtime-XAML tests; the `Key=` fallback was NOT needed. Note the extension's `ProvideValue` returns an observable-backed `IBinding` (see #7), not the plain `Binding` the plan sketched — the duck-typed `IBinding` return still wires to styled and direct target properties identically.
