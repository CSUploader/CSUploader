...
- [x] **Step 1-2:** Port both; wire `ShowHttpDetailsAsync` (`new HttpDetailsWindow(transaction).ShowDialog(await GetOwnerOrRevealAsync())` — Task-returning, completes on close, mirroring `DialogService.cs:156-164`). Gallery buttons: `DialogLogDetailsButton` (synth LogEvent), `DialogHttpDetailsButton` (synth transaction through the REAL service member).
- [x] **Step 3: Headless tests:** HttpDetails — synthesized transaction populates Summary/Timing/Proxy texts and all eight sub-tab boxes (assert a known substring per box: a header name, the pretty-printed JSON's indentation, a hex-dump offset column); null-transaction ctor path shows the fallback message (`HttpDetails_NoData`). LogDetails — fields bind (DateTime formatted via the converter, thread id, multi-line message in the Text tab).
- [x] **Step 4: Bridge session:** `logdetails` + `httpdetails` shots light+dark; ALSO drive the tab strip via the bridge (click the Response tab, then a sub-tab — the TabControl interaction proof) before capturing a second angle if useful. Contact sheet + Read pairs against the WPF references; this is where the Task 1 tab arbitration verdict gets its real-view confirmation.
- [x] **Step 5:** Full suite gate; record counts. **Commit** — `"feat(avalonia): LogDetails + HttpDetails windows (keyed-style→class rule; ShowHttpDetailsAsync wired)"`

**Task 7 executed 2026-07-11 (commit fd8e4d1):** Both windows ported per §Port rules; `ShowHttpDetailsAsync` flipped NotImplemented→real (reveal-or-own + `await ShowDialog`), so the service's throwing members are down to the pinned 3 Phase-5 editors. **Keyed-style rule (prep item 4) applied as specified:** WPF's window-local `FieldLabel`/`FieldValue` keyed styles (the latter `BasedOn={x:Type TextBox}`) became `<Window.Styles>` class selectors `TextBlock.field-label` / `TextBox.field-value` with **BasedOn dropped** — carrying only the four distinguishing setters (IsReadOnly, FontSize, Padding, SurfaceMutedBrush background); the class styles layer over Fluent's own TextBox theme and render faithfully (labels secondary, value boxes muted-fill). HttpDetails code-behind ported near-verbatim (framework-free `HttpTransaction` formatting; `System.Windows`→`Avalonia.Controls` only). 4 headless tests added (Avalonia 201→205: HttpTransaction populates all 8 sub-tab boxes + Summary/Timing/Proxy; NoData + message fallbacks; LogDetails OneWay field mapping incl. converted DateTime). WPF 1178/1178 unchanged, both heads 0-warning; scope 8 files, WPF head + Core untouched, zero resx. Bridge session: `logdetails`/`httpdetails` light+dark pairs match the WPF references — **only accepted divergence is the Fluent underline tab strip vs WPF's classic bordered tabs (Task 1 verdict, now real-view-confirmed on both dialogs, both themes)**. Nested-tab interaction PROVEN via `ava_action verb:select`: Request→Response main tab switched content to the response headers, then the Response Headers→Body(JSON) sub-tab switched to the pretty-printed JSON (evidence `_httpdetails-dark-response.png`, `_httpdetails-dark-response-json.png`). Note: HttpDetails has no Close button (WPF parity) and no IsCancel, so the bridge closed it via `ava_eval` (`Windows.Where(...).Close()`), not a button — a per-dialog closing wrinkle worth remembering for Phase 5+ windows that lack a Close affordance. DEV-LOOP HAZARD (Task 8): snapshot desktop.Windows to a list BEFORE calling Close() in an ava_eval drive — closing mid-enumeration throws 'Collection was modified'.
...
- [x] **Step 1-3:** Port the two windows; make the sink real; gallery buttons: `DialogUpdateProgressButton` toggles the sink through the REAL registered `IUpdateProgressSink` (first click `Open()`+`SetStatus(downloading 1.2.3)`+`Report(42)`, second click `Close()` — a toggle, so the bridge can close it); `DialogProgressButton` runs `ProgressWindow.ExecuteAsync(this, "Synthesized long operation", allowCancel: true, ct => Task.Delay(15000, ct))` — self-closing, and the bridge can click Cancel (which exercises the relabel + cts path). **DEVIATION (team-lead directed):** ProgressWindow ported MINIMAL — window shell + LabelText/CancelButton surface, NO ExecuteAsync (the orphaned-ExecuteAsync finding; keep-vs-delete goes to the maintainer at the gate, §Reality-check #13). So `DialogProgressButton` instead constructs the minimal window directly (two-line label + visible Cancel, the WPF reference-driver shape) and `Show(this)`es it non-modally for the shot; Cancel only relabels (no cts to signal).
- [x] **Step 4: Headless tests (sink):** under the headless lifetime `Open()` takes the ownerless branch — window shown; `SetStatus("x")`/`Report(42)` reflected in the controls (InternalsVisibleTo is in place, csproj:96); `Report` before `Open` is a no-op (null-window guard, WPF parity); `Close()` closes. Sink-focused (no ExecuteAsync to test): fresh-window-per-Open, Report updates bar+percent, full-lifecycle never-auto-close (WPF parity), and the hidden-owner guard (Show(owner) throws `InvalidOperationException` on a hidden owner — §Reality-check #12 CONFIRMED).
- [x] **Step 5: Bridge session:** `updateprogress` + `progress` shots light+dark (open via buttons, capture, close via toggle/Cancel — all bridge-driven). Contact sheet + Read pairs (ProgressBar look vs the Task 1 arbitration verdict).
- [x] **Step 6:** Full suite gate; record counts. **Commit** — `"feat(avalonia): UpdateProgress window + real IUpdateProgressSink; ProgressWindow ported (currently caller-less, flagged)"`

**Task 8 executed 2026-07-11 (commit `a732778`):** UpdateProgressWindow ported per §Port rules (CanResize=False, /Assets/icon.ico, ProgressBar consumed AS-IS per the Task 1 Fluent-blue verdict); `AvaloniaUpdateProgressSink` flipped from the 4-member no-op stub to real, mirroring `WpfUpdateProgressSink` — `Open()` creates a FRESH window each call (the interface's retry contract: replaces any prior surface, Report/SetStatus target the most-recent; previous window NOT closed = WPF parity), owner from `DialogOwnerResolver.ResolveFromLifetime()` with `Show(owner)` when non-null else ownerless `Show()`. **§Reality-check #12 CONFIRMED:** Avalonia's `Show(owner)` throws `InvalidOperationException` on a hidden owner exactly like ShowDialog (headless test `ShowWithHiddenOwner_Throws`), which is precisely why the sink leans on the resolver (only ever returns a visible window) and shows ownerless on null — it never passes a hidden owner. **ProgressWindow ported MINIMAL (team-lead deviation from the plan's honestly-async ExecuteAsync):** window shell + LabelText/CancelButton surface only, NO ExecuteAsync — the caller-less-primitive keep-vs-delete decision is the maintainer's at the Task 9 gate (§Reality-check #13); Cancel relabels+disables (WPF visible behavior) with no CancellationTokenSource. 7 headless sink tests added (Avalonia 205→212: ownerless-Open-under-headless, fresh-window-per-Open, Report-updates-bar+percent, full-lifecycle-never-auto-closes, Report/SetStatus-before-Open no-op, Close-closes+clears, hidden-owner-throws). WPF 1178/1178 unchanged; both heads 0-warning; scope 9 files (4 Avalonia head + 2 new windows + 1 test), WPF head + Core untouched, zero resx. Bridge session: `updateprogress`/`progress` light+dark pairs match the WPF references — **only divergence is the ProgressBar color (WPF unstyled OS-green → Fluent SystemAccent blue), the pre-recorded Task 1 verdict (accept AS-IS, no re-template)**. Also drove the minimal ProgressWindow's Cancel via the bridge → relabels to "Cancelling…" + disables (`_progress-light-cancelling.png`), proving the CancelButton surface. Non-modal `Show`/`Show(owner)` windows ARE addressable+screenshot-able by the bridge (extends the Task 1 modal-addressing verdict to non-modal). Gallery gained a Task 8 section (`DialogUpdateProgressButton` toggle through the REAL DI `IUpdateProgressSink`; `DialogProgressButton` direct minimal-window Show).
