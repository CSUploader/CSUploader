...
- [ ] **Step 1: Probe window.** A plain `Window` (900×500) holding one DataGrid with 3 text columns (Name/Size/URL) + `ClipboardCopyMode="IncludeHeader"`, fed in code-behind:
...
- [ ] **Step 4: Headless tests** (`GroupingProbeTests`, 3-4 `[AvaloniaFact]`s — these move to `UploadedViewTests` when Task 5 deletes the probe): view groups count == 3; group keys match; collapse API (`DataGrid.CollapseRowGroup(group, …)` — §Reality-check #1) hides the group's rows; re-expanding restores them.
