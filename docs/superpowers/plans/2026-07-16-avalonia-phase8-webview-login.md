...
**Phase 8 gate — reconcile ledger (recorded from the whole-diff gate panel):**

- **FIX (MAJOR) — login-window owner policy → visible-main-only.** `AvaloniaWebViewInteractiveAuthService.ResolveOwnerOrReveal` resolved the login window's owner via `DialogOwnerResolver.ResolveFromLifetime()` (active-visible-window-first, mirroring the picker path `AvaloniaDialogService.GetOwnerOrRevealAsync`). But this service is a LONG-LIVED, BACKGROUND-triggered modal: a burst of uploads can open one hoster's login while another hoster's login is already up and active, and Avalonia force-closes an owned window when its owner closes — so the second login got OWNED BY the first and was force-closed mid-captcha the instant the first completed (probe-proven: the second window's `ShowDialog` returned null silently). Fixed by resolving via `DialogOwnerResolver.ResolveVisibleMainOnly()` at BOTH sites (visible main window only, never a transient active sibling — the exact hazard `DialogOwnerResolver.cs:59-63` warns a long-lived surface must never take; WPF parity `WebViewInteractiveAuthService.cs:112` `Owner = MainWindow`). This is a DELIBERATE divergence from the picker path, which stays `ResolveFromLifetime` (a user-initiated picker correctly parents to the active window it was launched from). Regression test `LoginWindowsOwnedByMain_SurviveSiblingClose_WhereSiblingOwnedWouldDie` ties survival to the owner CHOICE (sibling-owned → force-closed, null result; main-owned → survives, own result); the policy-pick difference itself is already pinned by `DialogOwnerResolverTests` (`Resolve` returns the active sibling; `ResolveVisibleMainOnly` returns main). NOTE: the service's private resolver is desktop-lifetime-gated (returns null headless), so no test drives it directly — the mechanism test + resolver tests + the code change are the combined guard.
- **FIX (MINOR ×3 lenses, one root) — queued-marshal cancellation parity.** The WPF head passed the token into `Dispatcher.InvokeAsync` (`WebViewInteractiveAuthService.cs:71-74`), so a cancel that lands while the marshal is still queued aborts the invoke WITHOUT opening the window; the port ignored the token after `gate.WaitAsync(ct)`. Fixed with the panel's safe shape — inside the marshaled action, BEFORE starting `Pump`: `if (cancellationToken.IsCancellationRequested) { tcs.TrySetCanceled(cancellationToken); return; }`, plus a cheap `cancellationToken.ThrowIfCancellationRequested()` immediately after `gate.WaitAsync` (inside the `try`, so the `finally` still releases the gate). Deliberately NOT `ThrowIfCancellationRequested()` INSIDE the marshal: a throw there routes to the framework's unhandled-exception path (per `IUiDispatcher.InvokeAsync`'s contract), leaves the TCS unset, and hangs the call + the per-hoster gate PERMANENTLY. No cancellation registration that closes an OPEN dialog was added (neither head cancels an open dialog — parity). Regression test `CancelledWhileMarshalQueued_Throws_WithoutOpeningWindow` (a dispatcher double cancels-then-runs the marshaled action; Strict `IDialogService`/`ITrayIconService` mocks prove no window opened and no tray reveal).
- **RECORDED, no code change — parked-await at shutdown.** If the UI dispatcher loop exits before a queued marshal runs, the service's `await tcs.Task` parks harmlessly: the seam is async end-to-end (no blocked thread; `TaskCreationOptions.RunContinuationsAsynchronously`), so the process exits normally rather than deadlocking. Noted for completeness; no fix warranted.
- **PHASE 9 AGENT-SAFETY ADDITION (from a refuted-but-forwarded finding).** The Avalonia bridge (`--agent` + `EnableMutations`) can now reach REAL hoster network contact via the EditAccount **Sign in** button (an un-credentialed login-page GET through the live WebView), and ALWAYS could via **Check account** / **Refresh all accounts** (which POST the SEEDED FAKE credentials to real hoster endpoints). Phase 8 itself never drives these — agent-safety is unchanged this phase (no task opens a real login) — but Phase 9's cutover-bridge guidance MUST name **"Sign in"**, **"Check account"**, and **"Refresh all accounts"** as explicit DO-NOT-CLICK surfaces for any agent driving the live head. Separately, a deliberate Core-level `--agent` outbound-network latch (fail-fast / no-op real hoster contact when running under `--agent`) is surfaced as a **maintainer decision** — out of Phase 8 scope (Core is frozen this phase).

**Task 8 VERDICT (2026-07-15) — GATE PASSED, PHASE 8 CLOSED.** Whole-diff gate panel (3 lenses + mechanical gates + adversarial verify of every finding) ran on `phase7-shell-ready..HEAD`; it caught the owner-policy MAJOR + the cancellation MINOR above (fix commit `b7c2d9d`). Both gate-fix reviewers (`p8gatefix-reviewer`, `p8gatefix-reviewer-2`) independently returned **APPROVE** on `b7c2d9d`: FIX 1 both resolver sites flipped, WPF parity preserved, picker path deliberately unchanged, login-owned-by-login cascade now impossible; FIX 2 `TrySetCanceled`-not-throw verified against the real `AvaloniaUiDispatcher` (throw would hang the gate), gate released exactly once in all four cancel paths; both new tests non-vacuous (the cancel test empirically revert-checked → `MockException` without the guard; the owner test an honest mechanism/consequence guard, the policy PICK pinned in `DialogOwnerResolverTests`). Reviewer-2 empirically CONFIRMED the ledger network-hazard claim (Refresh/Check account POST `fake_rg_user`/`fake_catbox_user` to real Rapidgator/Catbox login endpoints via the live pipelines). The one non-blocking doc nit (ResolveVisibleMainOnly's XML doc + test header still framed as "non-modal only" despite the new modal login consumer) was folded in at commit `3d8a329`. Final: 3 files in `b7c2d9d` + 2 doc files in `3d8a329`; Avalonia **448/448**, WPF/shared **1201/1201**, forced-rebuild **0-warning**; Core-touch diff EMPTY; WPF-head diff EMPTY; `Strings*.resx` diff EMPTY. Tag `phase8-webview-login-ready` at `3d8a329`. Design doc reconciled (Phase 8 RECONCILED block, 10 items). **The Avalonia head is FEATURE-COMPLETE; only Phase 9 (staged cutover) remains.**
