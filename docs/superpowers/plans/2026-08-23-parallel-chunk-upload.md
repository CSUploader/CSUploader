...
3. `HttpHandler` materialises every response with `ReadAsStringAsync` before anything inspects it, so the completion parser's 64 KiB cap bounds the PARSE, not the allocation. A transport-wide response-size limit is separate hardening — worth doing, not part of this branch.
4. `OperationProgressEventArgs.cs:132` computes `DateTimeFinish` before the new `TimeRemaining` is assigned — a pre-existing bug Codex spotted, unrelated to this work but visible in the same code path.
