...
**Task 8 verdict — PHASE GATE PASSED (2026-07-15, tag `phase7-shell-ready`):** whole-diff review ran as a 14-agent panel (3 lenses + mechanical gates + adversarial verification). **All 9 mechanical gates PASS** on the first run (greps zero; suites 415/1201 pre-fix; i18n clean; Core-touch = exactly the ShowInfo 14 insertions; WPF-head = ReferenceShotCapture.cs only, `#if DEBUG`; Release builds + launch smoke clean; contact sheet 69 pairs with all 4 toast cells + the menu-bar recapture; column persistence verified live with the byte-identical serializer). **Panel: 10 findings → 8 confirmed (2 MAJOR), 1 clean-sweep note verified.** The MAJORs — both cross-task, both decompile/empirically proven, both FIXED at `143d233` (reviewer-approved with a full close-path-matrix walk): (1) the close-to-tray reroute vetoed Windows shutdown/logoff (Avalonia routes WM_QUERYENDSESSION through `Window.Closing` with an honored Cancel; the fix is the CloseReason early-return guarding `ApplicationShutdown or OSShutdown` — on Windows BOTH programmatic and OS session-end arrive as ApplicationShutdown); (2) tray restore re-fired `Window.Opened` (empirically proven headless) re-running the non-idempotent `InitializeAsync` (duplicate packages, N+1 log persistence) — fixed with the one-shot hydration guard, regression test pins the re-fire AND the guard together. MINORs fixed: the Ask-prompt task now observed (try/catch+log), `Close_WithExit_ActuallyCloses` now discriminates closed-vs-hidden, `AutoDismiss_StopsOnClose` got its try/finally. Recorded-only: the cross-head stranding ledger item + the info-toast accepted divergence (above). **Final counts: Avalonia 418/418, WPF/shared 1201/1201, both heads 0-warning Debug+Release.** Design doc reconciled (port rules 46/47/48: MenuItem.IsChecked OneWay default; the Closing shutdown-veto guard; the Opened re-fire one-shot rule). **The Avalonia head is feature-complete except Phase 8 WebView login.** maintainer-only checks outstanding: Win10 DWM dark-title-bar visual; 125%/150% DPI toast placement.
