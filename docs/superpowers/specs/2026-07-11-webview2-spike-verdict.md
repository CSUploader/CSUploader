...
Hosting a WebView2 `CoreWebView2Controller` in an Avalonia `NativeControlHost` child HWND works on Avalonia 11.3.18 / .NET 10 with `Microsoft.Web.WebView2` 1.0.4022.49. Every agent-verifiable point passed; **no point failed unfixably**, so the abort criterion (fall back to a separate WPF login-helper process) is **not** triggered. Two confirmations are `NEEDS-the maintainer` by nature — they can't be driven by a background agent — but neither reflects a hosting defect; they are final human sign-offs. Phase 8 proceeds with the in-process WebView2-in-Avalonia architecture.
...
|---|--------|--------|----------|
| a | Keyboard/typing into a real hoster login incl. Turnstile | **PASS (agent half) / NEEDS-the maintainer (native typing + Turnstile)** | WebView renders the live login page; JS focus + `Probe` confirm the email field (`INPUT id=input-9 type=email`) takes focus with a readable (empty) value. Native keystroke delivery could NOT be agent-verified — see below. |
...
| c | ShowDialog modal ownership | **PASS** | Modal-from-birth over the shown MainWindow. While open: `ava_windows` shows the dialog active + main inactive; **`IsWindowEnabled(ownerHWND) == False`** (native modality) while the modal stays enabled; after close the owner is re-enabled (`True`). |
...
- **(b) higher DPI:** the current monitor is at 100% (`RenderScaling` 1.00). The bounds math is `host DIP × RenderScaling`, DPI-general by construction, and passed cleanly at 100%. A background agent cannot change the system display scale. **the maintainer's confirmation needed:** run `…\ava\CSUploader.Avalonia.exe --agent --webview-spike` on a 125%/150% display (or drag it to a differently-scaled monitor), confirm the WebView still fills the host with no dead zones and corner links hit-test correctly.
...
- **Bounds:** drive `controller.Bounds` from host DIP × `RenderScaling` (finding 3); the child HWND `GetClientRect` is diagnostics-only.
