...
- **Phase 5 — Medium views**: UploadedView grouping FIRST (DataGridCollectionView risk probe), LogsView (zebra striping), EditProxyWindow, EditAccountWindow. SECURITY (Phase 2 gate note): the bridge redactor masks by VM property name and PasswordChar boxes — account views must bind ApiKey/SessionCookie/Password to PasswordChar TextBoxes or mark the controls [AgentHidden], else ava_props reads raw TextBox.Text.
