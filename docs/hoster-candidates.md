...
| **FileMirage** | bespoke (Laravel + dropzone) | The pick of what remains: its page declares `maxFileSize = 53687091200` (**50 GiB**) and an empty `api_token` for guests. No captcha. Would be a bespoke build. |
| **FEX.NET**, **DropMB**, **Bestfile**, **FilePort**, **FileGo**, **DooDrive**, **Rootz**, **eDisk**, **Imagenetz**, **GrosFichiers**, **Mega4Upload** | mixed | Alive, unclassified, all advertising ≤20 GB, none on a family with a base here. |
