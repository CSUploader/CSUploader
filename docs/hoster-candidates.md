...
| **megaup.net** | **YetiShare** | ⭐ **THE NEXT ONE.** Probed 2026-08-08: its uploader script declares `uploaderMaxSize = 5368709120` (**5 GiB**) and `maxChunkSize = 100000000`, **byte-identical to udrop/BowFile**, and `GET /assets/js/uploader.js` hands a **guest** `_sessionid` + `cTracker` + node with no login. A real guest upload to `f1NN.mupload.store/ajax/file_upload_handler` returned the full family envelope (`url`, `delete_url`, `short_url`). So it is a **thin shim on the existing `YetiSharePipeline`** — the fourth host on that base. |
