...
| 6 | **Sendspace** | **NEEDS A CAPTURE** | ⚠ **I was wrong to call this the lowest-risk build.** That rendered `upload_file[]` form is the no-JS fallback and posting to it fails; the real flow is a two-phase XHR (`/dragupload` → hash → form submit) whose first phase refuses every attempt, our own multipart writer included. See the Tier B row for the full protocol and the exact refusals. 300 MB cap. |
...
| **Sendspace** | sendspace.com | yes | **300 MB** | none | **NEEDS A CAPTURE** | ⚠ **Not Upstore-shaped — the visible form is a decoy.** Attempted 2026-08-01; the protocol is understood but no attempt was accepted. The homepage form (`upload_file[]` → `fsNNu…/upload?…&signature=…`) is the **no-JS fallback**, and posting to it answers `301 → /uploadprocerr.html?e=0` ("An error occurred while processing the uploaded file"). The real browser flow is **two-phase** (`dist/js/homepage.min.js`): (1) `POST <upload_form_drag_url>` — a `fsNNu…/dragupload?…` URL declared as a page global, body `FormData` with the file under **`fileField`**, plus headers `X-File-Name` (`unescape(encodeURIComponent(name))`), `X-File-Size`, `X-File-Type`, `X-File-ID` (client-side uid), `X-Dir-ID` (= the page's `upload_form_destination_dir`) → the response body is a bare **hash** string; (2) submit the form carrying `name[]`, `file[]` (the uid), `hash[]` (that hash) and `is_drag=1` → the result page with the links. Phase 1 refuses everything tried, with `403 Forbidden Request (invalid upload e:2)`, then `(invalid dir e:1)`, then 500s — **including through our own browser-shaped multipart writer**, so it is not a body-shape problem of the kind that bit 1fichier. Something session- or challenge-bound is missing. **Get a browser capture of one real upload before spending more.** Cap is authoritative, not "about 300": `upload_form_max_upload_size = 314572800` and the page's own message is "It must be under 300MB. Please upgrade to a Premium account". (Do **not** use the decommissioned `api.sendspace.com/rest/` XML API.) |
