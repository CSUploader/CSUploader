...
| **DropMeFiles** | dropmefiles.com | yes | **50 GB** | none | Medium | Alive, re-probed 2026-08-01. Corrections from the site's own words: **"Maximum size for the upload is limited up to 50 Gb, period of storage is up to 14 days"** — not the 100 GB / 25 days recorded here. Retention is chosen per upload: 1 download, 3, 7 or 14 days. ⚠ Temporary by design — a transfer service, not durable hosting. **Effort discount: it runs plupload** (`js/plupload2/`, `js/uploader.js`), the same uploader as Webshare — so the protocol reads straight out of its own script bundle rather than needing a capture. |
