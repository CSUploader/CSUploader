...
| ~~**Fileaxa**~~ | fileaxa.com | undeclared | CF-passive | high | **SHIPPED 2026-08-02** (FileaxaPipeline) — the plainest shim in the family: `/api/account/info?key=` and `/api/upload/server?key=` both answer `{"status":400,"msg":"Invalid key"}`, and `/login.html` serves a 200, so **no route overrides at all** (contrast Uploadrar). Account-only — no `utype=anon` form. `MaxFileSize` is null: the "10000 MB" recorded here is unverified and `?op=api_get_limits` answers with the homepage rather than the family XML, so there is no figure to trust — and the base's **1 GiB default would silently skip larger files**, which is the trap to watch for in every thin shim. |
...
| **Sharemods** | sharemods.com | **200 MB** | high | Probed 2026-08-02: the XFS REST API is live (`/api/account/info?key=` → `{"status":400,"msg":"Invalid key"}`), so it would be a thin shim — **but its homepage states "Maximum file size 200 Mb"**, not the "multi-GB" recorded here, which makes it near-useless for release parts. Also needs the Uploadrar treatment on routes: `/login.html` **403s**, `/login/` serves the form. Build only if 200 MB is worth having. |
