...
| **Account + captcha on every upload** | ~~depositfiles.com~~ (**wrong on both counts — SHIPPED 2026-08-09**: the Turnstile is on the LOGIN and only when the host feels like it, never on the upload, and `LoginInvalid` was simply what its API says to a caller with no session. See its row in the table below), ~~subyshare.com (premium-only upload)~~ (**wrong — SHIPPED 2026-08-11**: a FREE registered account uploads, no captcha anywhere. See its row in the table below) |
...
| ~~**Filecat**~~ | filecat.net | **2000 MiB acct** | CF-passive | — | **SHIPPED 2026-08-09 as an ACCOUNT host.** Not xfspro at all — a small JSON REST API, and **the whole of it is on `api.filecat.net`**: `POST /user/signin` (plain JSON, no captcha) → a `SESS` cookie, `POST /upldreq` → a node link, then one multipart `file` POST to it. 2000 MiB per file (stated nowhere — `upldreq` reveals it for free, since asking about a size uploads nothing), 2 GiB storage. ⚠ **The node needs the SESS cookie even though its request carries no auth header** — it is a `.filecat.net` domain cookie, and omitting it earns a 403 *after* the whole file has gone up. ⚠ Refusals arrive inside a **200** (`state:"rejected"`), and the link comes back **schemeless**. See `FileCatPipeline.cs`. |
...
Filenext (filenext.com), Filerio (filerio.in), Indishare (indishare.org), ~~Prefiles (prefiles.com — **SHIPPED 2026-08-11**, see below)~~, Mexashare (mexa.sh), Xubster (xubster.com), Hot4share (hot4share.com), Kenfiles (kenfiles.com), Filesfly (filesfly.cc), Silkfiles (silkfiles.com), Nelion (nelion.me), Filextras (filextras.com), Salefiles (salefiles.com), **Filespace** (filespace.com — *untrusted TLS chain, needs cert-validation relaxation like FlashBit/GigaPeta*).
...
| ~~**Depositfiles**~~ | depositfiles.com | no | **10 GiB** | Turnstile (login, **risk-triggered**) | — | **SHIPPED 2026-08-09** (DepositFilesPipeline) — account-only, three calls: `GET /api/upload/regular` (session cookie) → `{"upload_url":"https://fileshareNNNN…/FSNNN-1u/?X-Progress-ID=…","max_file_size_mb":"10240"}`, then ONE multipart POST to that node with `files` + `format=html5` + **`member_passkey`** + `fm`/`fmh` → `{"download_url","delete_url"}`. The POST target is the node URL with its query stripped and `/FS` rewritten to `/upload/FS` — the site's own script does exactly that. ⚠⚠ **The passkey is what files the upload under the account, and omitting it fails SILENTLY**: with a perfectly good session cookie but an empty `member_passkey` the upload still answers `{"status":"OK"}` with a working link, and the file is simply absent from the account's own `/api/file/listing` — proven both ways live. Same trap as FileMirage's wrong bearer token, so the pipeline refuses to upload without one. ⚠ **The captcha is on the LOGIN only, and it is risk-triggered, not per-login**: `POST /api/user/login` takes a plain username and password (its four captcha fields are posted empty by the site itself) and worked repeatedly, then the SAME request began answering `{"error":"CaptchaRequired","error_code":104}`. **That is why it ships on the browser rather than on a password**, in the session-cookie credential family: a sign-in that usually needs no window and occasionally does is worse for the user than one that always opens it — they can't predict which they'll get, and the surprise lands halfway through saving an account. The session it captures is issued with `Max-Age=31536000`, so the window is a once-a-year interruption. **The upload itself is never captcha-gated.** ⚠ The browser never sees the login JSON, which is where `member_passkey` normally arrives — hence the scrape off `/?upload=1` (`sharedkey`), which the upload path also uses whenever no key is stored. ⚠ Files expire **121 days** after upload on the free tier (`dt_expires` in the account listing). Anonymous does not exist: the node call answers a caller with no session `LoginInvalid`. Only `autologin` authenticates — the `al_<hash>` cookie set alongside it does not. See `DepositFilesPipeline.cs`. |
...
| ~~**Subyshare**~~ | subyshare.com | no | free account | 5 GB | Medium | ✅ **SHIPPED 2026-08-11 as an ACCOUNT host** — see its row in the shipped table below. The 2026-08-02 note here was right that it is not MoneyPlatform and wrong about the rest: it is XFileSharing, of an older vintage than the rest of the family. |
...
| ~~**filecat.net**~~, **filefox.cc**, **doodrive.com**, **fileport.io**, **rootz.io** | None answer `/server`, `op=start_upload` or `?op=api_get_limits`. SPA shells or sign-up-first sites; filecat and filefox also load reCAPTCHA. ⚠ **filecat.net was SHIPPED 2026-08-09 anyway** — this verdict was right (no anonymous route) for the wrong reason: **its entire API is on `api.filecat.net`**, which probing the apex could never find. **Before writing any of the others off as "no API", check for an `api.` subdomain.** |
...
| ~~**FileStore**~~ (filestore.me) | **SHIPPED 2026-08-10 as an ACCOUNT host — and it is the first host here that shipped *despite* a Cloudflare challenge.** ⚠⚠ **The apex is challenged and the upload nodes are not.** `filestore.me` and `www.filestore.me` answer this client `403` + `Cf-Mitigated: challenge` + `cType:'interactive'` on **every** route — sign-in, `?op=upload_form`, `?op=api_get_limits`, `/api/*`. `srvN.filestore.me`, where the bytes go, is **plain Apache with no Cloudflare at all**. So "Cloudflare-blocked" was true of the pages and false of the host. **What makes it shippable:** on this fork the form's `sess_id` IS the `xfss` cookie, so the sign-in browser — which passes the challenge, being a browser — can hand over everything an upload needs. Its `SuccessProbeScript` fetches `?op=upload_form` *inside the page* and returns `{node, sess}`; after that the app talks only to the node. Verified: a file posted straight to `srv9` with that session answered `[{"file_status":"OK","file_code":…}]` and the shipped pipeline uploads end to end. Account-only (`utype=anon` → *"uploads are not enabled for your account type"*), **250 MB** per file (`max_upload_filesize: '250'`). ⚠ Nodes are interchangeable but individually flaky — srv2/srv9/srv10 all took the same session while **srv1** answered `500 "Server don't allow uploads at the moment"` — so the node is captured at sign-in, not guessed. ⚠ **A lapsed session produces the SAME wording as an account-type refusal**, because an unknown `sess_id` just looks anonymous to the node; the pipeline names the likelier cause first. ⚠ Nothing can re-check the account from here (every page that could is behind the challenge), so a stored sign-in is kept until an upload disproves it. See `FileStorePipeline.cs`. **➡ Worth re-testing every "Cloudflare-blocked" row below against its UPLOAD NODE rather than its apex** — that distinction is untested for them, and it is the whole reason this one shipped. |
| ~~**kshared**~~ (www.kshared.com) | **SHIPPED 2026-08-10 as an ACCOUNT host — the THIRD site found on Emload's engine**, after jumploads, in a `/v1/` dialect: `POST /v1/account/signin` → `POST /v1/drive/get_server_for_upload` → one multipart POST to the node it names → `{kind:"fileSaved",file:{ID}}`; link `https://www.kshared.com/file/<file.ID>`. ⚠⚠ **The sign-in returns THREE tokens and each goes somewhere different**: `hash` (a JWT) is the `Authorization: Bearer`, `accesstoken` (a different JWT) is the body/multipart field `ud`, and `ut` (a base64 blob) is the field `ut`. A `PHPSESSID` from a prior `GET /` is needed on top. ⚠⚠ **Its two error reasons both name the wrong problem**: sending the accesstoken as the Bearer answers `sessionExpired` (on a sign-in seconds old), and sending the wrong value as the upload's `ut` answers **`disk`** — a storage message for a bad token, which cost real time here until the corrected request stored the same 2 MB file without complaint. Neither can be taken at face value, and the pipeline re-words both. ⚠ Its upload node also prints `Notice: Undefined index: HTTP_USER_AGENT …` **into the response body** when a request arrives without a User-Agent, which turns a successful upload into unparseable JSON; this app always sends one, and the parser now tolerates junk before the brace regardless, because reporting a *successful* upload as failed earns the user a duplicate file. Account-only; no per-file cap claimed (its pre-flight accepted 100 GB and nothing publishes a figure). Verified end to end through the shipped pipeline. See `KsharedPipeline.cs`. |
| ~~**PreFiles**~~ (prefiles.com) | **SHIPPED 2026-08-11 as an ACCOUNT host** — stock XFileSharing on the web-form path, **512 MB** per file, no Cloudflare challenge to this client, and a thin shim apart from its rewritten routes. ⚠ **Anonymous is refused, and its own API makes that look otherwise**: `?op=api_get_limits` answers a SIGNED-OUT caller in full — a node in `<ServerURL>` and `<MaxUploadFilesize>512</MaxUploadFilesize>` — which is exactly the seam that made UpZur and BtaFile anonymous. Posting the family's guest field set to that very node answers `[{"file_status":"uploads are not enabled for your account type","file_code":"undef"}]`. The 512 MB is the site's global figure, not a guest permission. ⚠ Its routes moved (`/login`, `/register`, `/my-account`, `/logout`), so the login page, the credential POST **and the account page** all needed pointing — while `?op=upload_form` and `?op=api_get_limits` still work. ⚠ It links a plain `/logout`, so the family's signed-in probe reported "signed in, but the account page didn't load as logged-in" after a sign-in that had just worked (the DDownload/DataNodes quirk, third time). ⚠ **Its header greets rather than names**: `<i class="fa fa-user"></i>Hi, <a …>Lynford Audie</a>!` — the family scrape saved every account as **"Hi"**, and the anchor beside it is a DISPLAY name while the host signs in with the EMAIL, so the shim reports no name at all and lets the typed address stand. See `PrefilesPipeline.cs`. |
| ~~**SubyShare**~~ (subyshare.com) | **SHIPPED 2026-08-11 as an ACCOUNT host** — XFileSharing of an **older vintage than anything else here**, **5 GB** per file, and the list's own "premium-only upload" was wrong: a **FREE** registered account uploads, with no captcha on the login or the upload. Sign-in is stock (`op=login` to the site root → 30-day `xfss`). Three things about the upload are its own, and all three fail silently if missed. ⚠ **The form's action is half-built by design** — it ends `upload.cgi?upload_id=` and the page's own script appends the id plus `js_on`, `utype`, `upload_type` and `usr_id`; posting the scraped action verbatim (the family default) sends the whole file to a request with no upload id. ⚠ **Its field set carries the account's numeric `usr_id` and the node's `srv_tmp_url`**, both hidden on the same page, while `utype` moves to the QUERY — and since a pipeline is a singleton, those page-scraped values are keyed per attempt (a new `BuildClassicExtraFields(ctx, sessId)` seam on the base) so two concurrent uploads can't trade them. ⚠ **The reply is not JSON**: `<textarea name='fn'>CODE</textarea><textarea name='st'>OK</textarea>` inside a self-submitting form. It is translated back into the family's `[{file_code,file_status}]` envelope by a new `NormalizeUploadResponse` hook rather than parsed locally, so the shared parser stays the only thing that knows `file_code:"undef"` means a discarded upload. ⚠ It links `/account/logout` (the DDownload/DataNodes/PreFiles quirk, fourth time) and its file manager carries no username, so the typed login name stands. ⚠ **It answers a client with no `User-Agent` a bodiless 403 on every route** — this app always sends one; a hand-built probe must too. Verified end to end through the shipped pipeline: the link resolves and its page names the file. See `SubysharePipeline.cs`. |
...
| ~~**BtaFile**~~ (btafile.com) | **SHIPPED 2026-08-09** — **anonymous at 100 MB or 10 GB signed in**, stock XFileSharing on the classic web-form upload. Never on any list here; it arrived as a user capture. ⚠ **Nothing on the site offers the guest upload** — there is no `utype=anon` form on the homepage or anywhere else, and `?op=upload` bounces a signed-out caller to the login page — yet the family's anonymous field set posted to the node it names answers `[{"file_code":…,"file_status":"OK"}]` with a live download page. The node and both caps come from the keyless `?op=api_get_limits` (100 without a session, 10240 with one), the same route UpZur needs, and this is the second host on that fork's theme — which is why its `div.freespace` storage bar moved onto the base. ⚠ Its upload form is on **`?op=upload`**, not the family's `?op=upload_form`: the family page exists and returns 200, it just renders no form, so pointing the base at it reports a perfectly good session as expired. ⚠ **Both of its form actions carry the URL-importer's `?upload_type=url`, including the file one** — kept verbatim rather than "corrected", because a capture shows a real file posted to exactly that action and accepted. No REST API (`/api/*` answers an HTML 404), so the account path is the web form; sign-in is a plain `op=login` post with no captcha (its `token` field is genuinely empty). See `BtaFilePipeline.cs`. |
...
| **TakeFile** (takefile.link) — *disabled in tree since 2026-06-28, re-checked 2026-08-11* | ❌ **Two independent reasons, and the second one is new.** (1) **The managed challenge is still there**: apex, `?op=upload_form`, `?op=api_get_limits`, `/api/upload/server` and `/user_login` all answer this client `403` + `cType:'managed'` — unchanged after six weeks, and cf_clearance forwarding cannot beat it (the wall is the TLS fingerprint, see `TakeFilePipeline.cs` and the 2026-06-28 note). (2) **A FREE account is offered no upload at all**, from a signed-in capture: its nav is Logout / My Account / **My Downloads** / Premium / Support — no upload link, no `my_files`, and the signed-in homepage is a pricing page with no uploader on it. Account type reads `FREE`; the only upload-shaped thing on the account page is `?op=my_account&generate_api_key=1`. **➡ This closes the "re-test the Cloudflare rows against their upload NODE" follow-up for this host specifically**: a node is only ever named by an upload form, and this account never gets one — so there is no node to ask. Would need a premium account AND a way through the challenge; don't re-attempt on either alone. |
...
| ~~**Emload**~~ | ✅ **SHIPPED 2026-08-09 as an ACCOUNT host — and "Cloudflare 403 to all fetches" was wrong.** Every path answers this client normally: the SPA pages, the API, the sign-in, the upload node. Whatever produced that 403 was not a wall. It is not XFS either but a bespoke SPA API: `POST /v2/app/user/signin` (`{em, passw, robo:"__"}`, no captcha) → `{uid, ut, ud, si}` where `ut` is a 7-day JWT; `POST /v2/app/drive/get_available_server` with the file's declared size → `{server:{ID, uri, token}}`; then ONE multipart POST to that node with `ui`/`ut`/`ud`/`ID`/`dir`/`server`/`token`/`file`. ⚠ **Four cookies authenticate it** — `__uid`, `__ut`, `__ud` AND `__si` — and **the SPA's own JavaScript sets them, so no `Set-Cookie` ever crosses the wire**; send three and every call answers `{"error":true,"reason":"oauth"}`, which reads like a rejected sign-in rather than a missing header. ⚠ **`Authorization: Bearer …` is NOT a credential**: the site's `post()` sends `__ha || randit(12)`, and a run of A's was accepted. ✅ **No guessed cap needed** — the node call is told the size and answers `reason:"disk"` when the account hasn't the room, so an impossible upload is refused *before any bytes move*; measured against a free account, 20 GiB was accepted and 50 GiB refused, and it is remaining storage rather than a per-file limit, so `MaxFileSize` is null. Anonymous doesn't exist (`reason:"oauth"` without a token). The link is the one its own script builds — `https://www.emload.com/v2/file/<file.**token**>`, the token not the ID, and the apex form without `/v2/` only redirects. ⚠ A sign-in was seen to fail once with a Cloudflare **520** on a request identical to ones that worked either side of it (seven consecutive sign-ins minutes later all succeeded), so the sign-in — and only the sign-in, which creates nothing — is tried twice. See `EmloadPipeline.cs`. |
| **Filextras** (filextras.com) | ❌ **Free accounts cannot upload — confirmed at the NODE, through its own API.** Stock XFileSharing; registration is open (hCaptcha) and sign-in works, but the signed-in `?op=upload_form` renders **no form at all**, only *"Oops! Uploads are disabled for your user type"*. ⚠ **That page was not taken as the answer**, because a blocked web form is not a blocked host — this list has been wrong that way before. Its **REST API is live**, and `?op=my_account` offers a one-click *"Generate API Key"*, so the API path was walked instead: `/api/account/info` answers `status:200` (and reports `premium_expire` already in the past for a fresh account), `/api/upload/server` answers `status:200` and names a rotating node (`panama.` / `clichester.filextras.com`) **together with its own `sess_id`** — and posting a file to that node, with that session, still answers `[{"file_code":"undef","file_status":"uploads are not enabled for your account type"}]`. So the gate is server-side and applies to both paths; it is the premium wall the family words this way (cf. Clicknupload, Easybytez), not a UI decision. ⚠ Worth repeating for the next reader: the first run of that probe posted the **API key** as `sess_id` instead of the session the server call returned — same refusal, but it would have been a refusal about the wrong thing. The recorded verdict is from the corrected request. Its `?op=api_get_limits` serves the homepage rather than the family XML, so there is no keyless route here either. Re-check only with a paid account. Supersedes its listing in the A3 sweep (there it was only noted as rendering no *anonymous* form). |
| **Filestore.to** (filestore.to) | ❌ **Uploading is PAID — not a gate to get past, a product to buy.** ⚠ **Not to be confused with `filestore.me`, which IS shipped** (see A4); similar name, unrelated site — this one is a small bespoke German host, not XFileSharing. Signed in on a free account, `/upload` renders no uploader at all, just the notice: *"Premium erforderlich — Für Uploads benötigst du einen aktiven Premium-Status"* ("Premium required — for uploads you need an active Premium status"), and `/konto` shows **Premium-Status: abgelaufen**. Upgrades are 20 days €10 / 75 days €25 / 200 days €50. ⚠ **The verdict here rests on the host's own page copy and the account state, NOT on a node's refusal** — unlike File.AL below, no upload endpoint is exposed to a free account, so there is nothing to post at and nothing to be refused by. Same class as subyshare.com. Re-check only if a paid account ever exists; the protocol was never mapped because nothing renders it. |
| **File.AL** (file.al) | ❌ **Upload is gated behind MANUAL per-account approval — a class of "no" this list hadn't met before.** Everything works right up to the bytes: registration is open (reCAPTCHA, `op=register`, and the 302 signs you straight in with `xfss`), `?op=upload_form` renders a complete form with a real `sess_id`, a real action (`titan.secureservercloud.net/cgi-bin/upload.cgi?upload_type=file&utype=reg`) and `max_upload_filesize: '20000'` (20 GB) — and the page then covers it with a modal: *"Uploads are enabled for confirmed users only. Please contact us for uploads approval."* ⚠ **A modal is a UI decision, so the node was asked directly** (2026-08-10, registered session still live, `?op=my_account` confirming signed-in). It gives the same answer, which makes this the server's verdict rather than the page's: `[{"file_code":"undef","file_status":"uploads enabled for approved users only. Please contact us for uploads approval"}]`. Note the shape — **HTTP 200, ordinary-looking XFS JSON, `file_code:"undef"`** — the same success-shaped failure DataVaults produced; anything reading only the status code would call that an upload. Stock XFileSharing otherwise, so **if a user ever gets approved this is a thin shim** and nothing needs reverse-engineering. Re-check only with an approved account; there is no seam to find without one. |
