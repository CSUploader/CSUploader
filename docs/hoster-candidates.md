...
| 6 | **Sendspace** | Medium | ✅ **Anonymous re-verified live 2026-08-01**: the homepage renders `<form action="https://fs13u.sendspace.com/upload?SPEED_LIMIT=0&MAX_FILE_SIZE=314572800&UPLOAD_IDENTIFIER=…&signature=…">` with `upload_file[]` and no captcha — a rotating node with a signed action, i.e. exactly the Upstore scrape-the-form shape. `MAX_FILE_SIZE` is 314572800 = 300 MB exactly. Lowest-risk build left; the cap is the only thing against it. (Do **not** use the decommissioned `api.sendspace.com/rest/` XML API.) |
