...
Uploads_Move_Down10               = 下移 10
Uploads_Tooltip_MoveUp            = 上移（更早上传）
Uploads_Tooltip_MoveDown          = 下移（更晚上传）
...
Settings_Upload_MaxParallelParts   = 每个文件的最大并行分片数
Settings_Upload_MaxParallelPartsTip= 仅适用于协议允许乱序发送文件分片的网盘。在按流量计费或不稳定的连接上请调低此值。
...
Wizard_Step2_Col_Captcha           = 下载需人机验证？
Wizard_Step2_Col_Captcha_Tooltip   = 无账号用户通过分享链接下载时是否需要进行人机验证。等待倒计时和自动执行的浏览器检查不计在内。
Wizard_Step2_Captcha_Unknown       = —
Wizard_Step2_Captcha_UnknownTooltip = 尚未确认该主机的下载流程是否需要人机验证。
...
Upload_SkippedNotSignedIn_Format = 已跳过 — 未登录 {0}。请在设置 → 账户中登录后重新开始上传。
Accounts_SessionExpired                    = 会话已过期，请重新登录。
Wizard_Step2_SessionExpiredTooltip         = 此账户的登录已过期。请在设置 → 账户中重新登录。
Wizard_Hoster_AccountSessionExpired_Format = {0}：“{1}”的登录已过期，因此不会上传到此处 — 请在设置 → 账户中重新登录。
...
Wizard_AccountCheckFailed_Format   = 无法登录 {0}，因此未添加该账户：{1}    # {0} = hoster name, {1} = verifier message
