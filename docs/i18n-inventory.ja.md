...
Settings_Upload_MaxParallelParts   = ファイルあたりの並列パート数の上限
Settings_Upload_MaxParallelPartsTip= パートを順不同で送信できるプロトコルのホスターにのみ適用されます。従量制または不安定な接続では小さくしてください。
...
Wizard_Step2_Col_Captcha           = DL時CAPTCHA？
Wizard_Step2_Col_Captcha_Tooltip   = アカウントにログインせず共有リンクからダウンロードする際に、CAPTCHAを解く必要があるかどうか。待ち時間やブラウザの自動チェックは含みません。
Wizard_Step2_Captcha_Unknown       = —
Wizard_Step2_Captcha_UnknownTooltip = このホストのダウンロード時にCAPTCHAが必要かどうかは未確認です。
...
Upload_SkippedNotSignedIn_Format = スキップしました — {0} にサインインしていません。設定 → アカウントでサインインしてから、再度アップロードを開始してください。
Accounts_SessionExpired                    = セッションの有効期限が切れました。再度サインインしてください。
Wizard_Step2_SessionExpiredTooltip         = このアカウントのサインインは期限切れです。設定 → アカウントで再度サインインしてください。
Wizard_Hoster_AccountSessionExpired_Format = {0}: 「{1}」のサインインが期限切れのため、アップロードされません。設定 → アカウントで再度サインインしてください。
